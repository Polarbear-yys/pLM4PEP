...
def combinefeature(pep, featurelist, dataset):  #把featurelist里包含的feature堆叠起来
    ...
    if 'bertfea' in featurelist:
        if len(pep) == 500:     #训练集
            f_bertfea = torch.load('./esm-reduced/train_dataset_500.pt')
            #加载预训练好的esm2模型
        ...
        if len(pep) == 607:     #独立测试集1
            f_bertfea = torch.load(
                './esm-reduced/independent_test_dataset1_607.pt')
            b = f_bertfea

        if len(pep) == 612:     #独立测试集2
            f_bertfea = torch.load(
                './esm-reduced/independent_test_dataset2_612.pt')
            b = f_bertfea

        if len(pep) == 1018:       #独立测试集3
            f_bertfea = torch.load(
                './esm-reduced/independent_test_dataset3_1018.pt')
            b = f_bertfea
...
def train(peptides, features, target, pickle_info, dataset):    #定义训练模型
    ...
    # pickle.dump(pickle_info, open("./model/LR-"+dataset+"-esm.pickle", "wb"))
